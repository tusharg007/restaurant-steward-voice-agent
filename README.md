...
The orchestrator owns the turn lifecycle: transcribe input, append history, build a prompt with current state, bind tools to the session, request reasoning, record the answer, update reference context, and pass the answer to mocked TTS. Only typed tools mutate order state; the LLM receives state as read-only context.
...
The five focused tests demonstrate:
...
logs/                    # Stable sample transcripts + runtime transcripts
```
